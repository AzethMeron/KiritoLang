...
// or invoke undefined behaviour. Each case must either compute a well-defined value or raise a
...
// Run and return the error message (empty if it did not raise).
...
int main() {
    ...
    // --- division / modulo by zero raise (never crash) ---------------------------------------
    {
        KiritoVM vm;
    ...
    // --- empty-sequence reductions raise rather than read out of bounds -----------------------
    {
        KiritoVM vm;
    ...
    // --- type mismatches in operators raise with an actionable, op-appropriate message --------
    {
        KiritoVM vm;
