...
int main() {
    // --- builtins on bad arguments raise cleanly (never crash) ---
    {
        KiritoVM vm;
        CHECK(raises(vm, "chr(-1)"));
        CHECK(raises(vm, "chr(1114112)"));      // one past U+10FFFF
        CHECK(raises(vm, "ord(\"\")"));
        CHECK(raises(vm, "ord(\"ab\")"));
        CHECK(raises(vm, "bin(\"x\")"));
        CHECK(raises(vm, "pow(2, 3, 0)"));
        CHECK(raises(vm, "isinstance(5, 42)"));
        CHECK(raises(vm, "divmod(1, 0)"));
        CHECK(raises(vm, "reversed(5)"));
        CHECK(raises(vm, "all(5)"));
        CHECK(raises(vm, "List(5)"));
        CHECK(raises(vm, "Dict([1, 2, 3])"));
    ...
    // --- unpacking with random shapes: counts always checked, never out-of-bounds ---
    {
        ...
        for (int i = 0; i < 200; ++i) {
            ...
            CHECK(raises(vm, src) == shouldFail);
        }
    }
    ...
    // --- deep recursion in module functions is bounded, not a stack smash ---
    {
        ...
        CHECK(raises(vm, "var ft = import(\"functools\")\n"
                         "var rec = Function(n):\n    return rec(n + 1)\nrec(0)\n"));
