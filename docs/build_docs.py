...
def rewrite_internal_links(body, slugs):
    ...
    def r(m):
        ...
        return m.group(0)
    return re.sub(r'href="([a-z0-9-]+)\.html(#[^"]*)?"', r, body)
...
def _group_of(slug):
    ...
    if slug in ("builtins", "types", "stdlib"):
        return "Reference"
    return "Guide"
...
a.xref{text-decoration:none;border-bottom:1px dotted rgba(167,139,250,.45)}
a.xref:hover{text-decoration:none;border-bottom-color:var(--accent)}
