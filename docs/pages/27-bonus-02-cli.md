...
## Parsing by hand gets old fast

You *can* pick apart `arglist` yourself, but even simple needs — an option with a value, a flag, a
default — turn into fiddly index juggling:

```kirito
var io = import("io")

# Imagine these came from the command line: a name, an optional --count, an optional --loud flag.
var args = ["Ada", "--count", "3", "--loud"]
...
while i < len(args):
    ...
    if tok == "--loud":
        ...
        i = i + 1
        count = Integer(args[i])     # remember to convert the String!
    else:
        name = tok
    i = i + 1

io.print(f"name={name} count={count} loud={loud}")
```

That works, but it's all boilerplate: no help text, no error on a typo'd option, no `--count=3`
form, and you had to remember the `Integer(...)` conversion. The `arg` module does all of this for
you.
