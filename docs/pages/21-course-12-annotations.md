# Lesson 12 — Type Annotations and Runtime Checks
...
wrap the call in a `try`/`catch` (Lesson 16 covers `try`/`catch` in depth) to see the annotation
error as a value.
