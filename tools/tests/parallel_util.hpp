...
// Like runScenario but expects the program to raise; returns the error message (empty if it did NOT
// raise — a test failure the caller asserts on).
...
// Run a program expected to raise an error whose message contains `needle`.
