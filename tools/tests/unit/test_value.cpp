...
int main() {
    ...
    CHECK(val(vm, 42).asInt() == 42);
    CHECK(val(vm, 3.5).asFloat() == 3.5);
    CHECK(val(vm, true).asBool() == true);
    CHECK(val(vm, std::string("hi")).asString() == "hi");
    CHECK(none(vm).isNone());

    // --- type queries ---
    CHECK(val(vm, 1).isInt() && val(vm, 1).isNumber());
    CHECK(val(vm, 1.0).isFloat() && val(vm, 1.0).isNumber());
    CHECK(val(vm, "x").isString());
    CHECK(val(vm, false).isBool());

    // --- typed-read errors are clear ---
    bool threw = false;
    try { val(vm, "not a number").asInt("arg"); } catch (const KiritoError& e) {
        threw = std::string(e.what()).find("expected Integer") != std::string::npos;
    }
    CHECK(threw);

    // asFloat accepts Integer or Float
    CHECK(val(vm, 7).asFloat() == 7.0);
    ...
    CHECK(d.get("missing", val(vm, -1)).asInt() == -1);   // default
    // pairs() round-trip
    int seen = 0;
    for (const auto& [k, v] : d.pairs()) { (void)v; if (k.asString() == "name" || k.asString() == "age" || k.asString() == "score") seen++; }
    CHECK(seen == 3);
    ...
    List big(vm);
    for (int i = 0; i < 50; ++i) big.add(val(vm, std::string("item") + std::to_string(i)));
    Value bigv = big.build();
    CHECK(bigv.len() == 50);
    CHECK(bigv.at(49).asString() == "item49");   // survived GC during construction
