...
//      / _setstate_): calling these with zero arguments must raise a catchable KiritoError, not read
...
//      raise; valid `\x`/`\u`/`\U` escapes still match.
//   5. complex factory negative-dim guards (stdlib_complex.hpp): complex.zeros / ones / identity
//      with a negative dimension raise (instead of a huge/garbage allocation).
#include <cmath>
#include <cstddef>
#include <cstdint>
#include <limits>
...
// True iff `src` raises (compile-time or run-time) on a fresh VM.
...
int main() {
    ...
    // ============================================================================================
    // 3) makeMethod zero-arg OOB guards — calling a 1-arg native method with NO args must raise a
    //    catchable KiritoError (it used to read a[0] out of bounds = UB).
    // ============================================================================================
    {
        // BytesIO.write() with no argument raises.
        ...
        // DateTime.format() with no argument raises.
        ...
        // DateTime._setstate_() with no argument raises (the deserialization hook; OOB-guarded).
        CHECK(throws("var tm = import(\"time\")\nvar d = tm.make(2020, 1, 2, 3, 4, 5)\nd._setstate_()"));
        {
            std::string e = errOf("var tm = import(\"time\")\nvar d = tm.make(2020, 1, 2, 3, 4, 5)\nd._setstate_()");
    ...
    // ============================================================================================
    // 4) regex hex-escape range — an above-U+10FFFF escape raises; valid escapes match.
    // ============================================================================================
    {
        const char* re = "var re = import(\"regex\")\n";
        // \UFFFFFFFF (8 F's = 0xFFFFFFFF) is way above U+10FFFF -> raises.
        CHECK(throws(std::string(re) + "re.compile(\"\\\\UFFFFFFFF\")"));
        // \U00110000 (one past U+10FFFF) -> raises (the exact boundary).
        ...
        // an incomplete escape (too few hex digits) raises too.
        CHECK(throws(std::string(re) + "re.compile(\"\\\\x4\")"));
    }

    // ============================================================================================
    // 5) complex factory negative-dim guards — negative dims raise (no garbage/huge allocation).
    // ============================================================================================
    {
        const char* cx = "var c = import(\"complex\")\n";
        // zeros / ones with a negative dimension raise.
        ...
        // identity with a negative dimension raises.
        ...
        // zero is a legal dimension (an empty matrix), not negative — must NOT raise.
