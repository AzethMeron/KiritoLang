...
int main() {
    // A live resource (Socket) can't cross a Queue: serialization raises cleanly instead of corrupting
    // the receiving arena.
...
if argmain:
    ...
    // Bounded queue mechanics: full/empty/qsize, putnowait-on-full and getnowait-on-empty raise, and
...
if argmain:
    ...
    // get/timeout and getnowait on an empty queue raise (no hang).
    expectError("getnowait empty raises", R"KI(
var parallel = import("parallel")
if argmain:
    var q = parallel.Queue()
    discard q.getnowait()
    ...
    expectError("get timeout raises", R"KI(
var parallel = import("parallel")
if argmain:
    var q = parallel.Queue()
    ...
    // A closed, drained queue raises on get (the worker-loop termination signal).
    expectError("get on closed raises", R"KI(
var parallel = import("parallel")
if argmain:
    var q = parallel.Queue()
    q.put(1)
