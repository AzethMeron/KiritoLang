...
// execution, so an undefined name is a COMPILE-time error (raised from runSource before any code runs)
...
int main() {
    ...
    // runtime "unbound" case — i.e. NOT a compile error ===
    {
        ...
        // at run time it is unbound and raises, which the try/catch handles.
