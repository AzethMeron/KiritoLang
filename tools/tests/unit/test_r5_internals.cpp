...
//   3. Resolver + analyzer from C++: an undefined name is a COMPILE-time error (raised from
...
//        - tensor.einsum with a repeated OUTPUT label ("ii->ii", "i->ii") raises a catchable
...
int main() {
    ...
    }
    {
        ...
        //  - a slotted local read before its own assignment, no outer binding -> raises (not garbage).
        CHECK(throws("var f = Function():\n return q\n var q = 1\nf()"));
    ...
    // ============================================================================================
    {
        // Undefined names are COMPILE-time errors (raised by runSource before any code runs).
    ...
    // ============================================================================================
    {
        // tensor.einsum with a REPEATED output label raises a catchable error (was an OOB buffer overrun).
    ...
    }
    {
        ...
        // zero, a trailing dot, and an empty exponent all parse rather than raising.
