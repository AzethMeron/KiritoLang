...
int main() {
    ...
    // ===== wrong-type arguments raise cleanly (no crash) =====
    CHECK(raises(vm, "import(\"hash\").md5(123)"));
    CHECK(raises(vm, "import(\"zlib\").compress([1, 2, 3])"));
    CHECK(raises(vm, "import(\"math\").sqrt(\"x\")"));
    CHECK(raises(vm, "import(\"json\").parse(42)"));
    CHECK(raises(vm, "import(\"matrix\").zeros(\"a\", 2)"));
    CHECK(raises(vm, "import(\"time\").make(\"y\", 1, 1)"));
    CHECK(raises(vm, "import(\"net\").urlencode(\"not a dict\")"));
    CHECK(raises(vm, "import(\"random\").Random(\"seed\")"));
    CHECK(raises(vm, "import(\"zlib\").decompress(\"not valid deflate data!!!\")"));

    // ===== wrong arity / unknown keyword (the signature layer) =====
    CHECK(raises(vm, "import(\"hash\").sha1()"));                       // missing arg
    CHECK(raises(vm, "import(\"hash\").sha1(\"a\", \"b\")"));            // too many
    CHECK(raises(vm, "import(\"math\").sqrt(x=1, bogus=2)"));            // unknown keyword
    CHECK(raises(vm, "import(\"matrix\").identity()"));                 // missing required
    ...
    // ===== fuzz: random programs over the modules must never crash; only raise catchable errors =====
