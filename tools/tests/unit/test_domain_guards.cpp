...
static bool raised(const std::string& src) { return !err(src).empty(); }
static bool has(const std::string& hay, const std::string& needle) {
    return hay.find(needle) != std::string::npos;
}

int main() {
    // === math: domain errors raise; NaN passes through; overflow-to-inf is NOT a domain error ===
    CHECK(has(err("discard import(\"math\").sqrt(-1)"), "math domain error"));
    CHECK(has(err("discard import(\"math\").log(0)"), "math domain error"));
    CHECK(raised("discard import(\"math\").log(-1)"));
    CHECK(raised("discard import(\"math\").asin(2)"));
    CHECK(raised("discard import(\"math\").acos(2)"));
    CHECK(raised("discard import(\"math\").acosh(0)"));
    CHECK(raised("discard import(\"math\").atanh(1)"));
    CHECK(raised("discard import(\"math\").log2(0)"));
    CHECK(raised("discard import(\"math\").log10(0)"));
    CHECK(raised("discard import(\"math\").log1p(-1)"));
    CHECK(raised("discard import(\"math\").gamma(0)"));
    CHECK(raised("discard import(\"math\").gamma(-2)"));
    CHECK(raised("discard import(\"math\").pow(-2, 0.5)"));   // negative base, fractional exponent
    CHECK(raised("discard import(\"math\").pow(0, -1)"));     // zero to a negative power
    CHECK(raised("discard import(\"math\").fmod(1, 0)"));     // divisor zero
    CHECK(raised("discard import(\"math\").log(8, 1)"));      // base 1
    // NaN passes through, overflow→inf does not raise, valid input still computes
    ...
    // prod overflow raises like its siblings; a Float in the mix never overflows
    ...
    CHECK(has(err("discard import(\"complex\").pow(import(\"complex\").zero, import(\"complex\").of(0-1,0))"),
              "zero to a negative"));
    CHECK(raised("discard import(\"complex\").zero ** import(\"complex\").of(0-1, 0)"));
    CHECK(raised("discard import(\"complex\").atanh(import(\"complex\").one)"));
    // valid in the complex plane — must NOT raise
    CHECK(!raised("discard import(\"complex\").sqrt(import(\"complex\").of(0-1, 0))"));
    CHECK(!raised("discard import(\"complex\").log(import(\"complex\").of(0-1, 0))"));
    CHECK(!raised("discard import(\"complex\").acosh(import(\"complex\").zero)"));
    ...
    CHECK(raised("discard import(\"tensor\").Tensor([0.0]).log()"));
    CHECK(raised("discard import(\"tensor\").Tensor([2.0]).asin()"));
    CHECK(raised("discard import(\"tensor\").Tensor([2.0]).atanh()"));
    CHECK(raised("discard import(\"tensor\").Tensor([0.5]).acosh()"));
    CHECK(has(err("discard import(\"tensor\").Tensor([0.0]).reciprocal()"), "division by zero"));
    CHECK(raised("discard import(\"tensor\").Tensor([0-2.0]).pow(0.5)"));
    CHECK(raised("discard import(\"tensor\").Tensor([1.0]).clip(5.0, 1.0)"));
    ...
    CHECK(raised("discard Integer(\"--5\")"));
    CHECK(raised("discard Integer(\"+ 5\")"));
    CHECK(raised("discard Integer(\"0x-5\")"));
    ...
    CHECK(raised("discard import(\"statistics\").quantiles([1,2,3], 0)"));
    CHECK(raised("discard import(\"statistics\").quantiles([1,2,3], 0-2)"));
    ...
    // === time.strptime: out-of-range fields + trailing data raise; make() rollover unchanged ===
    CHECK(raised("discard import(\"time\").strptime(\"2024-99-99\", \"%Y-%m-%d\")"));
    CHECK(raised("discard import(\"time\").strptime(\"2024-01-01XYZ\", \"%Y-%m-%d\")"));
