...
//   * filesystem mutation: mkdir / remove / mkremove / rename / chmod
...
// leniency: `mkdir(exist_ok=False)` raises if the directory already exists; `remove`/`mkremove`
// (missing_ok=False) raise if the target does not exist. `mkdir`/`remove`/`mkremove` return a Bool
// (True = it did the work, False = the opt-in lenient no-op); `rename` returns None and raises on
...
public:
    ...
    void setup(ModuleBuilder& m) override {
        ...
        // --- mutation (strict by default: raise rather than silently no-op) ---
        ...
        // it removes something. A non-empty directory raises ("directory not empty") — use mkremove.
        ...
        // mkremove(path, missing_ok=False) -> Bool: recursively delete a directory and everything under
        ...
        m.fn("mkremove", {{"path", "String"}, {"missing_ok", "Bool", vm.makeBool(false)}}, "Bool",
             [pathArg](KiritoVM& vm, std::span<const Handle> a) -> Handle {
            std::string p = pathArg(vm, a[0]);
            bool missing_ok = Value(vm, a[1]).asBool("missing_ok");
            std::error_code ec;
            auto count = std::filesystem::remove_all(p, ec);   // number of entries deleted (0 = absent)
            if (ec) throw KiritoError("mkremove: " + ec.message());
            if (count == 0 && !missing_ok)
                throw KiritoError("mkremove: '" + p + "' does not exist (pass missing_ok = True to ignore)");
        ...
        // bits are meaningful). Lenient (returns success as a Bool; a missing file is False, no raise).
